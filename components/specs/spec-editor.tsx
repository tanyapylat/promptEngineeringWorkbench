...
export function SpecEditor({ projectId }: { projectId: string }) {
  ...
  const [pendingEditVersionId, setPendingEditVersionId] = useState<
    string | null
  >(null);
  ...
  async function handleContentUpdate(content: SpecContent) {
    ...
    if (selectedVersion.status === 'committed') {
      ...
      try {
        ...
        setSelectedVersionId(newVersion.id);
        toast.success(
          `Created draft v${newVersion.version} from v${selectedVersion.version}`,
        );
      } catch (error) {
  ...
  async function handleDiscardAndCreateNew() {
    ...
    try {
      ...
      setSelectedVersionId(newVersion.id);
      toast.success(
        `Created new draft v${newVersion.version} from v${targetVersion.version}`,
      );
    } catch (error) {
    ...
    <TooltipProvider>
            ...
            <div className="flex flex-col gap-3">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    Describe this version (like a commit message) - required to
                    commit
                ...
                {isEditable && (
                  <Tooltip>
                    ...
                    <TooltipContent>
                      <p className="text-xs">
                        Freeze this version and make it immutable
                      </p>
                    </TooltipContent>
                  </Tooltip>
                )}
              ...
              {!isEditable && (
                <div className="rounded-md border border-blue-200 bg-blue-50 dark:border-blue-900 dark:bg-blue-950/30 p-3">
                  <p className="text-sm text-blue-800 dark:text-blue-200">
                    📝 This is a committed version (read-only). Start editing
                    any field below to create a new draft from this version.
                  </p>
                </div>
              )}
              <SpecJsonEditor
      ...
      {draftSpec && pendingEditVersionId && (
        <DraftConflictDialog
          ...
          selectedVersion={
            versions.find((v) => v.id === pendingEditVersionId)?.version ?? 0
          }
